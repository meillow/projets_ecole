...
<div align=center>
    Quelle question souhaitez-vous modifier ?

    <FORM action='<?php echo site_url('user/modification_question')?>' method='post'>
      
      <SELECT name='modification' size='1'>
        <?php 
        $ques_numero=$this->session->userdata('ques_numero');

        foreach($ques_numero as $value=>$key){ // affichage du nombre de question 
            $valuetemp=$value+1;
...
<div align=center>
  ...
  <table class='quizz'>
    ...
    <?php $clé_quizz=$this->session->userdata('quizz_clé'); //chargement données des questions des quizz
      $ques_libele=$this->session->userdata('ques_libele') ;
      $ques_image=$this->session->userdata('ques_image') ;

      foreach ($ques_numero as $value=>$key){ //affichage question une à une
        $valuetemp=$value+1;
        ...
        foreach ($data2 as $value2=>$key2){ //affichage des réponses une à une
          ...
          if($statut==="1"){//couleur question change en fonction de si vraie ou fausse
